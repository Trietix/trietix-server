...
export const createInvite =  catchAsync(async (req: Request, res: Response) => {
    ...
    sendMail(req.body.email, "You've been invited as a volunteer - Trietix", { oraganizer: user?.name, eventTitle: event?.title, eventVenue: event?.venue, eventDate: event?.date, eventLocation: event?.location, eventTime: event?.time, url: `https://trietix.vercel.app/invites/${invite?.id}` }, "user/invite.hbs")
