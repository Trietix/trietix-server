...
const sendMailBlast = (eventId, eventDay) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let event = yield event_model_1.default.findById(new mongoose_1.default.Types.ObjectId(eventId), 'title image organizer location venue');
        if (event) {
            let eventName = event.title;
            let prep_emails = yield ticket_1.ticketModel.find({}, 'email');
            let emails = (0, sendMail_1.removeDuplicateEmails)(prep_emails);
            // await sendMail("emmanuelomoiya6@gmail.com", `TRIETIX - ${eventName} - 🔥 Don't Miss Out! Get Your Party Ticket Now! 🎟️`, { eventTitle: event.title, eventPoster: event.image, eventLink: `https://trietix.com/events/${event.title.split(" ").join("-")}`, eventDay: eventDay }, "user/mail-blast.hbs", true, ["emmanuelomoiya6@gmail.com", "omoiyaemmanuel@yahoo.com"], 2)
            yield (0, sendMail_1.sendMail)("emmanuelomoiya6@gmail.com", `TRIETIX - ${eventName} - 🔥 Don't Miss Out! Get Your Party Ticket Now! 🎟️`, { eventTitle: event.title, eventPoster: event.image, eventLink: `https://trietix.com/events/${event.title.split(" ").join("-")}`, eventDay: eventDay }, "user/mail-blast.hbs", true, emails);
            return { eventName };
        }
        else {
            throw new errors_1.ApiError(http_status_1.default.BAD_REQUEST, "Event not found!");
        }
    }
    catch (e) {
        throw new errors_1.ApiError(http_status_1.default.FORBIDDEN, e);
    }
});
exports.sendMailBlast = sendMailBlast;
/**
 * Get all the events
 * @returns {Promise<IEventDoc | null>}
 */
const getEvents = () => __awaiter(void 0, void 0, void 0, function* () {
    const events = yield event_model_1.default.find({ isEnded: false, isCancelled: false }).sort({ createdAt: -1 });
    if (!events) {
