...
exports.resetPassword = exports.forgotPassword = exports.refreshAuth = exports.logout = exports.loginUserWithEmailAndPassword = void 0;
...
const forgotPassword = (email) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let user = yield (0, user_service_1.getUserByEmail)(email);
        if (user) {
            let resetToken = yield (0, token_service_1.generateResetPasswordToken)(email);
            if (user.bank) {
                (0, sendMail_1.sendMail)(email, "Reset your Trietix password", { resetToken: resetToken, email: email }, "user/reset-password.hbs");
            }
            else {
                (0, sendMail_1.sendMail)(email, "Reset your Trietix password - Organizer", { resetToken: resetToken, email: email }, "organizer/reset-passwor.hbs");
            }
        }
        else {
            throw new ApiError_1.default(http_status_1.default.UNAUTHORIZED, "You don't have an account!");
        }
    }
    catch (error) {
        throw new ApiError_1.default(http_status_1.default.FORBIDDEN, error);
...
const resetPassword = (resetToken, password) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const resetTokenDoc = yield (0, token_service_1.verifyToken)(resetToken, token_types_1.default.RESET_PASSWORD);
        let newPassword = yield bcryptjs_1.default.hash(password, 8);
        let user = yield (0, user_service_1.updateUserById)(new mongoose_1.default.Types.ObjectId(resetTokenDoc.user), { password: newPassword });
        if (!user) {
            throw new Error();
        }
        ;
        yield token_model_1.default.deleteOne({ token: resetToken });
        const tokens = yield (0, token_service_1.generateAuthTokens)(user);
        return { user, tokens };
    }
    catch (error) {
